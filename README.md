...
当前同步主项目的 v2.1 口径，但仍保持轻量仓库定位：

- policy 语义中心是 6 个 `DialogueAct`：`Greet / Elicit / Inform / Recommend / Reassure / Hold`。
- `T-state` 和旧 `A*` 动作只作为兼容标签保留。
- labeled JSON 同时包含旧字段 `candidate_actions / best_action` 和新字段 `schema_version / dialogue_act / act_params / realization`。
...
Backfill existing labeled files to v2.1 fields:
