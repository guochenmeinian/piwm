...
| `persona` | string | 一句话顾客画像 |
| `aida_stage` | enum | `attention / interest / desire / action` |
| `bdi` | object | belief / desire / intention 各一句话 |
| `interest_level` | string | 购买兴趣强度（自然语言） |
| `hesitation_level` | string | 犹豫程度（自然语言） |
...
## Kling Prompt（Step 3 产出）

`data/prompt/piwm_<id>.md` — 纯文本，填充自 labeled JSON 的行为字段 + timeline，直接传 Kling API。
