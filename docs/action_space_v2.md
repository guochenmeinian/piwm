...
- 主项目 `PIWM-Train-Synth-v2` 是 543 parent 的 schema v2.2 独立导出；本仓库只同步 action/schema 相关部分，不替代那批训练集。
- 两边共享 6 个 `DialogueAct`，但 realization 主体不同：本仓库生成 terminal response，主训练集保留 human-salesperson behavior。
- v2.2 在轻量仓库新增 `candidate_action_specs / best_action_spec / action_key / action_instance_key / outcomes_by_action_key`，旧 `candidate_actions / best_action / outcomes` 继续保留。
- `action_key` 对齐主项目的 canonical `(act, params)` key；`action_instance_key` 只在轻量仓库中用于区分多个旧动态 A-action 映射到同一 canonical key 的情况。
...
- v2.2 不再把辅助动作写成顶层 `co_acts`；新格式写入 `act_params.supporting_acts`，旧格式回溯字段为 `legacy_co_acts`。
...
- `script/action_space_v2.py`: act enum、兼容映射、supporting_acts 规范化、stable action key、realization 模板。
